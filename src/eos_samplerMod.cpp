...
struct LowDensityBridge {
    ...
    double p_end = 0.0;          // MeV/fm^3
    double n_end_over_ns = 0.0;  // n / n_s
    double mu_end_gev = 0.0;     // GeV
    double cs2_end = 0.0;
    double pCET = 1.0;
};
...
std::vector<double> sample_monotonic_increasing_cs2(
    int n_points,
    double cs2_start,
    double cs2_end_min,
    double cs2_end_max,
    ...
    if (n_points < 1) {
        throw std::runtime_error("n_points must be positive in low-density cs2 bridge");
    }

    const double end_min = std::max(cs2_end_min, cs2_start);
    const double end_max = cs2_end_max;

    if (!(end_max >= end_min)) {
        return {};
    }

    const double cs2_end = uniform(rng, end_min, end_max);

    std::vector<double> cs2(static_cast<std::size_t>(n_points));
    ...
    // Sample the interior points below the endpoint, then sort.
    // This guarantees:
    //   cs2_start <= cs2[0] <= ... <= cs2[n_points - 1] = cs2_end
    for (int i = 0; i < n_points - 1; ++i) {
        cs2[static_cast<std::size_t>(i)] = uniform(rng, cs2_start, cs2_end);
    }

    std::sort(cs2.begin(), cs2.end() - 1);
...
bool valid_eos_vectors(const EOSSample& sample) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        if (sample.e[i] <= 0.0 || sample.p[i] < 0.0 ||
            sample.n[i] <= 0.0 || sample.mu[i] <= 0.0) {
            return false;
        }

        if (sample.cs2[i] < 0.0 || sample.cs2[i] > 1.0) {
            return false;
        }
    }

    for (std::size_t i = 1; i < n; ++i) {
        // Energy density must increase for TOV interpolation.
        if (!(sample.e[i] > sample.e[i - 1])) {
            return false;
        }

        // Pressure may be constant at a first-order phase transition,
...
LowDensityBridge build_low_density_cs2_bridge(
    const Config& cfg,
    const CrustTable& crust,
    const CEFTTable& ceft,
    std::mt19937& rng
) {
    LowDensityBridge bridge;

    const double p0 = crust.p.back();                  // MeV/fm^3
    const double n0_over_ns = crust.n_over_ns.back();  // n / n_s
    const double n0 = n0_over_ns * ns;                 // fm^-3
    const double mu0 = crust.mu.back();                // MeV
    const double cs20 = crust.cs2.back();

    if (!(cfg.n_cet > n0_over_ns)) {
        throw std::runtime_error(
            "n_cet must be larger than the crust matching density. "
            "Increase n_cet or check n_crust."
        );
    }

    const int n_points = cfg.n_low_cs2;

    std::vector<double> cs2 = sample_monotonic_increasing_cs2(
        n_points,
        cs20,
        cfg.low_cs2_end_min,
        cfg.low_cs2_end_max,
    ...
    bridge.e.resize(static_cast<std::size_t>(n_points));
    bridge.p.resize(static_cast<std::size_t>(n_points));
    bridge.n.resize(static_cast<std::size_t>(n_points));
    bridge.mu.resize(static_cast<std::size_t>(n_points));
    bridge.cs2.resize(static_cast<std::size_t>(n_points));
    ...
    for (int i = 0; i < n_points; ++i) {
        ...
        const double n_over_ns =
            n0_over_ns + frac * (cfg.n_cet - n0_over_ns);

        const double n_now = n_over_ns * ns;

        const double cs2_now = cs2[static_cast<std::size_t>(i)];
        const double cs2_prev = (i == 0)
            ? cs20
            : cs2[static_cast<std::size_t>(i - 1)];

        const double cs2_mid = 0.5 * (cs2_prev + cs2_now);

        const double mu_now =
            mu_prev * std::pow(n_now / n_prev, cs2_mid);

        const double p_now =
            p_prev
            + 0.5 * (mu_prev * cs2_prev + mu_now * cs2_now)
              * (n_now - n_prev);

        const double e_now =
            mu_now * n_now - p_now;

        bridge.e[static_cast<std::size_t>(i)] = e_now;
        bridge.p[static_cast<std::size_t>(i)] = p_now;
        bridge.n[static_cast<std::size_t>(i)] = n_now;
        bridge.mu[static_cast<std::size_t>(i)] = mu_now;
        bridge.cs2[static_cast<std::size_t>(i)] = cs2_now;

        const double med = linear_interp(n_now, ceft.n, ceft.median_p);
        const double sig = linear_interp(n_now, ceft.n, ceft.sigma_p);

        if (sig > 0.0) {
            pCET_accum += std::exp(
            -std::pow(p_now - med, 2.0) / (2.0 * std::pow(sig, 2.0))
            );
            ++pCET_count;
        }
    ...
    bridge.p_end = bridge.p.back();
    bridge.n_end_over_ns = cfg.n_cet;
    bridge.mu_end_gev = bridge.mu.back() / 1000.0;
    bridge.cs2_end = bridge.cs2.back();
    bridge.pCET =
        pCET_count > 0 ? pCET_accum / static_cast<double>(pCET_count) : 1.0;
...
EOSSample make_candidate(
    ...
    LowDensityBridge bridge = build_low_density_cs2_bridge(cfg, crust, ceft, rng);
    ...
    if (cfg.verbose) {
    std::cout << "bridge endpoint: p=" << bridge.p_end
              << ", mu=" << bridge.mu_end_gev
              << ", cs2=" << bridge.cs2_end
              << ", pCET=" << bridge.pCET
              << '\n';
    }

    //if (bridge.p_end < cfg.p_cet_soft || bridge.p_end > cfg.p_cet_stiff) {
    //    return sample;
    //}

    // The existing high-density sampler starts at the endpoint of the
    // low-density sampled c_s^2 bridge.
    n_boundary[0] = cfg.n_cet;
    p_boundary[0] = bridge.p_end;
    m[0] = bridge.mu_end_gev;
    cseg[0] = bridge.cs2_end;

    sample.pCET = bridge.pCET;
    ...
    for (std::size_t i = 0; i < bridge.e.size(); ++i) {
        rows.push_back({
            bridge.e[i],
            bridge.p[i],
            bridge.n[i],
